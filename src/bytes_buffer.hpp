...
struct bytes_view {
    ...
    bytes_view subspan(size_t start, size_t len = static_cast<size_t>(-1)) const {
        if (start > size()) {
            throw std::out_of_range("bytes_view::subspan");
        }
        if (len > size() - start) {
...
struct bytes_buffer {
    ...
    bytes_const_view subspan(int start, int len) const {
        return operator bytes_const_view().subspan(start, len);
    }

    bytes_view subspan(int start, int len) {
...
struct static_bytes_buffer {
    ...
    operator bytes_const_view() const noexcept {
        return bytes_const_view{data(), N};
    }
